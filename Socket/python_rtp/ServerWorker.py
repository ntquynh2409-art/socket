...
class ServerWorker:
		...
		while True:
			...
			if data: 
				...
				try:
					...
					# Nếu frame lớn hơn MAX_RTP_PAYLOAD, cắt nó ra
					...
					while start_pos < total_len:
						...
						end_pos = min(start_pos + self.MAX_RTP_PAYLOAD, total_len)
						...
						self.clientInfo['rtpSocket'].sendto(
							self.makeRtp(payload, self.rtpSequenceNumber, marker),
							(address, port)
						)
	...
	def makeRtp(self, payload, frameNbr, marker):
		...
		extension = 0
		cc = 0
		# marker được dùng từ tham số truyền vào, không set cứng nữa
		pt = 26 
		seqnum = frameNbr
		ssrc = 0 
		
		rtpPacket = RtpPacket()
		# Truyền marker vào hàm encode của RtpPacket
